...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/':
          ...
          html.append('<div style="display:flex;align-items:center;justify-content:space-between"><h1 style="margin:0">OP25 Control</h1><div><button onclick="logout()" style="padding:8px 12px;border-radius:6px;background:#e67e22;color:#fff;border:0;cursor:pointer">Logout</button></div></div>')
          ...
          return
            ...
            # Prompt browser to clear Basic Auth cache by returning 401 with WWW-Authenticate
            try:
                self.send_response(401)
                self.send_header('Content-Type', 'application/json')
                self.send_header('WWW-Authenticate', 'Basic realm="OP25"')
